...
3. **Create the database.** Open the **SQL Editor** and run the files in
   ...
   9. `supabase/migrations/0012_group_fund_program_gender.sql` — **Care-of per section + Group Fund + Umrah gender**: `affiliates.program` (hajj / umrah / both), `affiliates.fund_mode` (individual / group_fund) with the auto-created `account_head_id` fund ledger, and `umrah_passengers.gender`. Care-of lists, the Group Fund toggle and the Male/Female filter on Umrah need this; the app degrades gracefully (all care-ofs shown in both sections, gender not stored) until it is applied.
   10. `supabase/seed.sql` — blog posts, site settings, menus, gallery (optional but recommended)

   **Group Fund (group leaders):** on a Care-of record switch *Money handling* to **Group Fund**. A ledger head "<name> (Group Fund)" is created; money from the leader is recorded on the Care-of page (**Receive fund**) as a credit, and every package assigned to a pilgrim under that Care-of is debited from the same head. The Care-of page shows total received, package charges and the advance / due, with the full ledger and a printable statement. Pilgrims under a Group Fund carry no individual due and their profile's payment form points to the fund.
